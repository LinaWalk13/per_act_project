...
pacman::p_load(devtools, tidyverse, dplyr, ggplot2, jsonlite, tidyr, car, rstatix, lme4, fitdistrplus, extraDistr, brms, stats, ggpubr, rstatix, boot)
library(pupilr)
library(eyer)
...
```
```{r}
fix_classes <- fix_classes[, c("recording.id", "proportion")]
# info_fix <- merge(info_fix, fix_classes, by = "recording.id") # running this doubles the rows in the data
```
...
log_model_filtered <- glm(duration ~ condition + (1 | id), data = info_fix_filtered, family = poisson(link = "log"))
summary(log_model_filtered)

# or this? 
mixed_model_filtered <- lmer(duration ~ condition + (1 | id), data = info_fix_filtered)
summary(mixed_model_filtered) 
