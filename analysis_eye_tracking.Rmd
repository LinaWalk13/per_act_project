...
gamma_model_log <- glm(log(duration) ~ condition + (1 | id), data = info_fix, family = Gamma(link = "log"))
...
```{r models_freq}
freq_gamma <- glm(fixation.freq ~ condition, family = Gamma(link = "log"), data = info_fix)
summary(freq_gamma)

freq_log <- glm(log(fixation.freq) ~ condition, data = info_fix)
summary(freq_log)

freq_inverse <- glm(fixation.freq ~ condition, inverse.gaussian, data = info_fix)
summary(freq_inverse)


AIC(freq_gamma, freq_log, freq_inverse)
```
The gamma models seems to fit best
```{r plot_freq_model}
plot(freq_gamma)
```
...
The gamma model was chosen based on AIC criteria and suggests the following based on the coefficients: 
  ...
  - Higher fixation frequency for condition H2 (0.1231, p < 2e-16).
  - Lower fixation frequency for conditions L1 (-0.2179, p < 2e-16) and L2 (-0.0700, p < 2e-16) compared to the reference.
...
This Generalized Linear Model (GLM) was fitted to the proportion of focal to ambient fixations using a Gamma distribution and a log link function. The model suggest: 
  ...
  - No significant difference for conditions H2 and L2 compared to the reference (p > 0.05).
  - A significant increase in the proportion for condition L1 (0.7516, p = 0.000321).
...
#fixation duration
info_fix %>%
  ggplot(aes(x = log(duration), fill = condition)) +
  geom_density(alpha = 0.5) +
  labs(title = " Log-transformed Fixation Duration Distribution",
       x = "Log (Fixation Duration)",
       y = "Density") +
...
### maybe use different plots, for example boxplots and so on
## espeiaclly the last one is quite ugly 


fix_classes %>%
  ggplot(aes(x = condition, y = 100, fill = as.factor(prop_focal_to_ambient > 1))) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("darkmagenta", "cornflowerblue"), name = "Proportion") +
  labs(title = "Bar Plot of Proportion of Focal to Ambient Fixations Across Conditions",
       x = "Condition",
       y = "Percentage",
       fill = "Proportion") +
  theme_bw() +
