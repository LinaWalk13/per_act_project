...
upper_threshold <- 2500 # change based on litertaure 
...
fix_classes <- fix_classes[, c("recording.id", "prop_focal_to_ambient", "fix_count", "focal", "ambient")]
id_rec_id <- info_fix_filtered[, c("id", "recording.id", "condition")]
...
# with logit link 
log_model <- glm(duration ~ condition + (1 | id), data = info_fix_filtered, family = poisson(link = "log"))
summary(log_model)  


gamma_model <- glm(duration ~ condition + (1 | id), data = info_fix_filtered, family = Gamma(link = "log"))
summary(gamma_model)

gamma_model_log <- glm(log(duration) ~ condition + (1 | id), data = info_fix_filtered, family = Gamma(link = "log"))
...
```{r AIC}
AIC(log_model, gamma_model, gamma_model_log)
```
 ...
 **Poisson Regression with Log Link:**
   - glm model with a log link function.
   - Identified that the gamma model with log-transformed duration performed the best based on AIC comapred to otther models
   - Coefficients showed significant effects of conditions H2, L1, and L2 compared to the reference condition H1.
    - Condition H2: Showed a decrease in log-transformed fixation duration compared to the reference condition H1.
    - Condition L1: Indicated an increase in log-transformed fixation duration.
    - Condition L2: Also showed an increase in log-transformed fixation duration, though to a lesser extent.

## models for fixation frequency 
```{r models_freq}
freq_gamma <- glm(fixation.freq ~ condition, family = Gamma(link = "log"), data = info_fix_filtered)
summary(freq_gamma)

freq_log <- glm(log(fixation.freq) ~ condition, data = info_fix_filtered)
summary(freq_log)

freq_inverse <- glm(fixation.freq ~ condition, inverse.gaussian, data = info_fix_filtered)
summary(freq_inverse)


AIC(freq_gamma, freq_log, freq_inverse)
```
The log models seems to fit best
```{r plot_freq_model}
plot(freq_log)
```
Not sure if this looks alright? 

### fixation frequency

The gamma model was chosen based on AIC criteria and suggests the following based on the coefficients: 


# model for proportion 

```{r}
prop_gamma <- glm(prop_focal_to_ambient ~ condition, family = Gamma(link = "log"), data = fix_classes)
summary(prop_gamma)

prop_log <- glm(log(prop_focal_to_ambient) ~ condition, data = fix_classes)
summary(prop_log)

prop_inverse <- glm(prop_focal_to_ambient ~ condition, inverse.gaussian, data = fix_classes)
summary(prop_inverse)
```


```{r prop_AIC}
AIC(prop_gamma, prop_log, prop_inverse)
```
Once again, the gamma model fits best. 
```{r plot_prop_model}
plot(prop_gamma)
```
is this alright? 
...
This Generalized Linear Model (GLM) was fitted to the proportion of focal to ambient fixations using a Gamma distribution and a log link function. The model suggest: 
...
# proportion 
fix_classes %>%
  mutate(ambient_prop = ambient / fix_count * 100,
         focal_prop = focal / fix_count * 100) %>%
  ggplot(aes(x = condition)) +
  geom_bar(aes(y = 100, fill = as.factor(prop_focal_to_ambient > 1)), position = "fill", stat = "identity") +
  scale_fill_manual(values = c("darkmagenta", "cornflowerblue"),
                    name = "Proportion",
                    labels = c("Ambient", "Focal")) +
  labs(title = "Proportion of Focal to Ambient Fixations Across Conditions",
       x = "Condition",
       y = "Percentage",
       fill = "Proportion") +
...
## not sure this is correct? but Like the idea of the plot
...
```{r}
task_model <- glm(task.duration ~ condition, data = info_fix_filtered)
```




## this is all stupid because we do have repeated measures but was what I thought we could do before I realised it :DDD
## Proportion of focal to ambient fixations 
...
# unequal variance, so we probably need to do something else and the anova above is useless? 


## apparently this is suitable if unequal variances are present
...
## Analysis of fixation frequency 

```{r freq_norm_test}
shapiro_test(info_fix$fixation.freq)
hist(info_fix$fixation.freq)
## tried every transformation I found but nothing made it normal 
```

```{r var_test}
leveneTest(fixation.freq ~ condition, data = info_fix)
```
```{r}
## this test is chosen as variances are unequal and we have four groups
## maybe there are other options of tests, I didn't read up on methods 1 here :O
kruskal_result_freq <- kruskal.test(fixation.freq ~ condition, data = info_fix)
print(kruskal_result_freq)# Kruskal-Wallis test

#  post hoc pairwise comparisons again
posthoc_dunn_freq <- dunn.test(info_fix$fixation.freq, g = info_fix$condition, method = "bonferroni")
print(posthoc_dunn_freq)
```
